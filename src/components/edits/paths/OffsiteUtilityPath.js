...
class OffsiteUtilityPath extends Component {

  constructor(props) {
    super(props);
    this.state = props.offsite_utility;
    ...
    return (
      <div className="path-with-input">
        ...
        <select className="utility" name={"utility"} value={this.state.utility} onChange={ this.handleChange }>
          <option value="undefined"></option>
          {this.props.data.offsite_utilities.map((ut, k) => 
        ...
        <button className="remove" onClick={() => this.props.removePath( { type: "offsite_utilities", index: this.props.index } )}>&#x2716;</button>       
      </div>
    );
...
export default OffsiteUtilityPath;