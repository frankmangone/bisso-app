...
class PurchasedUtilityPath extends Component {

  constructor(props) {
    super(props);
    this.state = props.purchased_utility;
  ...
  handleChange (event) {
    ...
    if (name.substring(0,4) === "path") {
      ...
      aux.utility = parseInt(event.target.value);
    }
    else if (name === "value"){
  ...
  render() {
    return (
      <div className="path-with-input">
        ...
        {/* Utility */}
        <select className="utility" name={"utility"} value={this.state.utility} onChange={ this.handleChange }>
          <option value="undefined"></option>
          {this.props.data.purchased_utilities.map((ut, k) => 
            <option key={k} value={k}>{ut.name}</option>                
          )}
        </select>
        <br />
        <input type="number" step="any" name="value"
               value={this.state.value} onChange={ this.handleChange }/>
        <button className="remove" onClick={() => this.props.removePath( { type: "purchased_utilities", index: this.props.index } )}>&#x2716;</button>       
      </div>
    );
...
export default PurchasedUtilityPath;